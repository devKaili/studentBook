...
void menu() { // main menu
    ...
    if (input_verification(5, menu_select_one)) { // if the user selected an available option, the program will proceed
        menu_select_confirmation(menu_select_one);
    } else { // if the user selected an unavailable option, the program will spit out an error and ask for menu select again
        printf("\n\n\033[1;31mZadana hodnota neodpovida moznostem v nabidce, zkuste to znovu...\033[0;37m\n\n");        
        sleep(2);
        menu();
    }
}
void menu_select_confirmation(int menu_select_one) { // print out user's input and ask if he/she wants to continue or change the option
    char menu_option[50];
    int menu_select_two;
    system("CLS");
    ...
    if (input_verification(2, menu_select_two)) { // if the user selected an available option, the program will proceed
...
int input_verification(int max_value, int input) { // check if user input exceeds min and max values
...
void greet_the_user() { // greet the user according to current time and his/her name
    char greet[50];
